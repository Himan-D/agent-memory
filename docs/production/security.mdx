...
```bash
# 1. Create new key
NEW_KEY=$(curl -s -X POST https://api.hystersis.ai/admin/api-keys \
...
curl -H "X-API-Key: $HYSTERESIS_API_KEY" https://api.hystersis.ai/health

# 5. Delete old key
curl -X DELETE https://api.hystersis.ai/admin/api-keys/$OLD_KEY_ID \
  -H "X-API-Key: $ADMIN_KEY"
```
