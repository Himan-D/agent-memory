...
**Example:**
```bash
curl -X POST https://api.hystersis.ai/agents \
...
**Example:**
```bash
curl -X GET "https://api.hystersis.ai/agents?status=active&department=support&limit=10" \
  -H "X-API-Key: your-api-key"
```
