...
# Env: NEO4J_*, QDRANT_*, OPENAI_API_KEY (embeddings)
# Optional LLM judge: EVALUATOR_API_KEY / COMPRESSION_VERIFY_API_KEY

# Full official LoCoMo (no mock)
./scripts/run-real-benchmarks.sh locomo

# Faster slice (still real data + live stores)
...
3. Every question scored by a configured **LLM judge** (`evaluator_configured: true`)
