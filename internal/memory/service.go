...
func NewService(cfg *config.Config) (*Service, error) {
	...
	// Assign concrete stores to interfaces only when non-nil. A nil *Client
	// placed directly into an interface is not == nil (typed nil), which would
	// bypass nil checks and panic on method calls.
	...
	svc := &Service{
		graph: graph, vector: vector, neo4jClient: neo, config: cfg, apiKeys: neo,
	}
