...
func TestNewService_UnavailableStoresUseNilInterfaces(t *testing.T) {
	cfg := &config.Config{
		Neo4j:  config.Neo4jConfig{URI: "bolt://127.0.0.1:59999", User: "neo4j", Password: "test"},
		Qdrant: config.QdrantConfig{URL: "http://127.0.0.1:59999"},
	...
	if err := svc.PingNeo4j(ctx); err == nil {
		t.Fatal("expected neo4j ping error when store unavailable")
	...
	if err := svc.PingQdrant(ctx); err == nil {
		t.Fatal("expected qdrant ping error when store unavailable")
