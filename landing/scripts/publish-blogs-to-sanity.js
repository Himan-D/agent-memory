/**
 * Publish blog posts to Sanity with cover images, inline images, and SEO fields.
 *
 * Usage (from landing/):
 *   set -a && source .env.local && set +a
 *   npm run blogs:publish
 *
...
async function publishBlog(blog, index) {
  ...
  if (existing?._id) {
    ...
    console.log(`   ✅ Updated ${existing._id} (${body.length} blocks, SEO fields set)`)
    return 'updated'
  }
...
async function main() {
  ...
  for (const p of verify) {
    console.log(`  • ${p.slug}: seo=${!!p.seoTitle} cover=${p.hasCover} blocks=${p.bodyBlocks} images=${p.inlineImages}`)
  }
