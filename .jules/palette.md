...
## 2024-06-09 - Add ARIA label to Clear Search button
**Learning:** Icon-only buttons within transient UI elements (like search dropdowns) often lack aria-labels because they are conditionally rendered and visually obvious to sighted users, but completely opaque to screen readers.
**Action:** Always verify `aria-label` attributes on icon-only buttons, especially those that dismiss or clear transient content.
