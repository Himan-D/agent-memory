...
## 2026-06-10 - [Parallel Multi-Query Retrieval]
**Learning:** Sequential processing of expanded search queries (prospection-guided retrieval) introduced latency scaling linearly with the number of expansions (O(N) queries).
**Action:** Batch embedding generation and parallelize vector searches using goroutines. Always move deterministic transformations (like filter map construction) outside concurrent loops to minimize redundant work.
