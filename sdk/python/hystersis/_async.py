...
class AsyncHystersis:
    ...
    async def relations_delete(self, relation_id: str) -> Dict[str, str]:
        """Delete a relation."""
        return await self.request("DELETE", f"/relations/{relation_id}")
    ...
    async def admin_cleanup(self) -> Dict[str, int]:
        """Delete all expired memories."""
        return await self.request("POST", "/admin/cleanup")
    ...
    async def admin_analytics(self) -> Dict[str, Any]:
        """Get admin analytics."""
        return await self.request("GET", "/admin/analytics")
    ...
    async def memories_create_link(
        self,
        from_id: str,
        to_id: str,
        link_type: str,
        ...
        payload = {"from_id": from_id, "to_id": to_id, "type": link_type, "weight": weight}
        if metadata: payload["metadata"] = metadata
        return await self.request("POST", "/memories/links", json=payload)

    async def memories_get_links(self, memory_id: str) -> List[Dict[str, Any]]:
        """Get all memory links for a memory."""
        return await self.request("GET", f"/memories/{memory_id}/links")

    async def memories_delete_link(self, link_id: str) -> Dict[str, str]:
        """Delete a memory link."""
        return await self.request("DELETE", f"/memories/links/{link_id}")

    async def memories_get_versions(self, memory_id: str) -> List[Dict[str, Any]]:
        """Get all versions of a memory."""
        return await self.request("GET", f"/memories/{memory_id}/versions")

    async def memories_restore_version(
        self,
        memory_id: str,
        version_id: str,
    ) -> Dict[str, str]:
        """Restore a memory to a previous version."""
        return await self.request("POST", f"/memories/{memory_id}/restore", json={"version_id": version_id})

    # ==================== Analytics ====================

    async def memories_get_stats(
        self,
        user_id: Optional[str] = None,
        org_id: Optional[str] = None,
    ) -> Dict[str, Any]:
        ...
        if user_id: params["user_id"] = user_id
        if org_id: params["org_id"] = org_id
        return await self.request("GET", "/memories/stats", params=params)

    async def memories_get_insights(
        self,
        user_id: Optional[str] = None,
        org_id: Optional[str] = None,
    ) -> List[Dict[str, Any]]:
        ...
        if user_id: params["user_id"] = user_id
        if org_id: params["org_id"] = org_id
        return await self.request("GET", "/memories/insights", params=params)

    async def memories_get_summary(
        self,
        user_id: Optional[str] = None,
        org_id: Optional[str] = None,
    ) -> Dict[str, Any]:
        ...
        if user_id: params["user_id"] = user_id
        if org_id: params["org_id"] = org_id
        return await self.request("GET", "/memories/summary", params=params)
...
class Hystersis:
    ...
    def __getattr__(self, name: str):
        ...
        compat_map = {
            ...
            'get_memory_links': 'memories_get_links',
            'get_memory_versions': 'memories_get_versions',
            'restore_memory_version': 'memories_restore_version',
            # Entities (old -> new)
            ...
            # Admin (old -> new)
            'cleanup_expired_memories': 'admin_cleanup',
            ...
            # Analytics (old -> new)
            'get_memory_stats': 'memories_get_stats',
            'get_memory_insights': 'memories_get_insights',
            'get_memory_summary': 'memories_get_summary',
            # Hybrid/Advanced search (old -> new)
            'hybrid_search': 'memories_hybrid_search',
            'advanced_search': 'memories_advanced_search',
            # Compaction (old -> new)
            'run_compaction': 'memories_run_compaction',
            'get_compaction_status': 'memories_get_compaction_status',
            # Misc (old -> new)
            'infer_memory': 'create_memory',  # Simplified
